...
from .av1_extractor import Av1Extractor
...
class Av1Dataset(Dataset):
    def __init__(self,
        ...
        if cached_split is not None:
            ...
            self.file_list = list(self.data_folder.glob("*.csv"))
            self.load = False
        else:
...
def collate_fn(batch):
    ...
    data["theta"] = torch.cat([b["theta"].reshape(1) for b in batch])
