...
class ModelMAE(nn.Module):
    def __init__(self,
        ...
        self.future_pred = nn.Linear(embed_dim, future_step * 2)
    ...
    def agent_random_masking(
        ...
        pred_masks = ~future_padding_mask.all(-1)  
        fut_num_tokens = pred_masks.sum(-1)  
    ...
    def lane_random_masking(x, future_mask_ratio, key_padding_mask):
        ...
        num_tokens = (~key_padding_mask).sum(1)  
    ...
    def forward(self, data):
        hist_padding_mask = data["x_padding_mask"][:, :, :20]
        hist_feat = torch.cat(
            [
                data["x"],
        ...
        future_padding_mask = data["x_padding_mask"][:, :, 20:]
        ...
        y_hat = self.future_pred(future_token).view(-1, 30, 2)  
        ...
        if not self.training:
            ...
            out["hist_pred_mask"] = hist_pred_mask
            out["future_pred_mask"] = future_pred_mask
            out["lane_pred_mask"] = lane_pred_mask
